...
import org.training.core.jalo.EmptyCartRemovalCronJob;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public EmptyCartRemovalCronJob createEmptyCartRemovalCronJob(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.EMPTYCARTREMOVALCRONJOB );
			return (EmptyCartRemovalCronJob)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating EmptyCartRemovalCronJob : "+e.getMessage(), 0 );
		}
	}
	
	public EmptyCartRemovalCronJob createEmptyCartRemovalCronJob(final Map attributeValues)
	{
		return createEmptyCartRemovalCronJob( getSession().getSessionContext(), attributeValues );
	}
